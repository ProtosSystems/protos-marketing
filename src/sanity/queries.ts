...
const TAGS_QUERY = defineQuery(/* groq */ `*[
  _type == "tag"
  && count(*[_type == "post" && defined(slug.current) && ^._id in tags[]._ref]) > 0
]|order(title asc){
  title,
  "slug": slug.current,
  "count": count(*[_type == "post" && defined(slug.current) && ^._id in tags[]._ref]),
}`)
...
const TAG_POSTS_QUERY = defineQuery(/* groq */ `*[
  _type == "post"
  && defined(slug.current)
...
export async function getPostsByTag(tag: string) {
  ...
  type TagPostRecord = {
    tags?: PostTagRecord[] | null
  }

  return {
    ...result,
    data: ((result.data ?? []) as TagPostRecord[]).filter((post) =>
      Array.isArray(post.tags) &&
      post.tags.some((postTag: PostTagRecord) => postTag?.slug === tag),
    ),
  }
}
